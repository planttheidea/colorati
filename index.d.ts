...
interface OpaqueColoratiOptions extends BaseColoratiOptions {
    alpha?: false;
    alphaPrecision?: undefined;
}
interface SemiOpaqueComputedColoratiOptions extends BaseColoratiOptions {
    alpha: true;
}
interface SemiOpaqueManualColoratiOptions extends BaseColoratiOptions {
    alpha: number;
}
type AlphaType = 'computed' | 'ignored' | 'manual';
type ColoratiOptions = OpaqueColoratiOptions | SemiOpaqueComputedColoratiOptions | SemiOpaqueManualColoratiOptions;
interface OpaqueColorConfig extends Required<OpaqueColoratiOptions> {
    alphaType: 'ignored';
}
interface SemiOpaqueComputedColorConfig extends Required<SemiOpaqueComputedColoratiOptions> {
    alphaType: 'computed';
}
interface SemiOpaqueManualColorConfig extends Required<SemiOpaqueManualColoratiOptions> {
    alphaType: 'manual';
}
type ColorConfig = OpaqueColorConfig | SemiOpaqueComputedColorConfig | SemiOpaqueManualColorConfig;
type NormalizedConfig<Options extends ColoratiOptions> = Options['alpha'] extends number ? SemiOpaqueManualColorConfig : true extends Options['alpha'] ? SemiOpaqueComputedColorConfig : OpaqueColorConfig;
type NormalizedOptions<Config extends ColorConfig> = Config extends SemiOpaqueComputedColorConfig ? SemiOpaqueComputedColoratiOptions : Config extends SemiOpaqueManualColorConfig ? SemiOpaqueManualColoratiOptions : OpaqueColoratiOptions;
type AnalogousColors<Config extends ColorConfig> = Tuple<Colorati<Config>, 6>;
type ComplementColors<Config extends ColorConfig> = Tuple<Colorati<Config>, 2>;
type ClashColors<Config extends ColorConfig> = Tuple<Colorati<Config>, 3>;
type NeutralColors<Config extends ColorConfig> = Tuple<Colorati<Config>, 6>;
type SplitColors<Config extends ColorConfig> = Tuple<Colorati<Config>, 3>;
type TetradColors<Config extends ColorConfig> = Tuple<Colorati<Config>, 4>;
type TriadColors<Config extends ColorConfig> = Tuple<Colorati<Config>, 3>;
type Tuple<Type, Length extends number> = Length extends Length ? number extends Length ? Type[] : TupleOf<Type, Length, []> : never;
type TupleOf<Type, Length extends number, Rest extends unknown[]> = Rest['length'] extends Length ? Rest : TupleOf<Type, Length, [Type, ...Rest]>;
type Value<Tuple extends any[]> = [...Tuple, alpha: number];
...
declare class Hex<const Config extends ColorConfig> extends BaseColor<Config> {
...
declare class Colorati<const Options extends ColoratiOptions> extends BaseColor<NormalizedConfig<Options>> {
    private _ansi16;
    private _ansi256;
